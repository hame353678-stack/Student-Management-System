...
export default function AdminShowQuizes({ Quize }: { Quize: QuizeType }) {
  return (
    <div className="border-2 border-red-500 rounded-xl p-5 max-w-sm mx-auto my-3 bg-green-300 hover:shadow-md hover:border-slate-300 transition-all duration-200">
      ...
      <form action={DeleteQuize}>
        <input type="hidden" name="coursname" />
        <input type="hidden" name="QuizeTitle" />
        <input type="hidden" name="InstructorName" />
        <button type="submit">Delete Quize</button>
      </form>
    </div>
  );
}
